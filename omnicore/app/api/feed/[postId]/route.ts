...
export async function GET(
  ...
  const { data: post, error: postErr } = await supabase
    .from("social_posts")
    .select(
      `*, vendor:vendors(id, name, slug, location, is_verified), product:products(id, title, price, image_url)`
    )
    .eq("id", postId)
    .maybeSingle();
...
export async function POST(
  ...
  const banned = ["http://", "https://", "bit.ly", "free money", "send me"].map((s) =>
    s.toLowerCase()
  );
  const lowered = body.toLowerCase();
  for (const b of banned) {
    if (lowered.includes(b)) {
      return NextResponse.json({ error: "Comment looks like spam." }, { status: 400 });
    }
  }
  ...
  if ((count ?? 0) >= 5) {
    return NextResponse.json(
      { error: "You are commenting too quickly. Slow down." },
      { status: 429 }
    );
  }
